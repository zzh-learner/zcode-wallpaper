    ...
    async function handle(req, res) {
      ...
      if (p === "/api/action" && method === "POST") {
        ...
        req.on("end", async () => {
          ...
          // applySkin/removeSkin: skin subsystem (spec §4.6). Uses server-side
          // require('./skin-inject.cjs') NOT spawn — theme is a structured object
          // in the request body. NO mutual exclusion with wallpaper anymore
          // (spec §overlay): skin + wallpaper coexist via color-mix frosted-glass panels.
          // Test env has no ZCode -> skin-inject throws -> {accepted:false}.
          if (req2.action === "applySkin") {
            ...
            // apply skin (no wallpaper removal — they coexist now)
            (async () => {
              let log = "";
              try {
          ...
          // NOTE: injectImage/injectVideo no longer remove skin — they coexist
          // now (spec §overlay: wallpaper + skin via rgba panels).
