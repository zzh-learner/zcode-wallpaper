...
const hindsight = require("./hindsight.cjs");
...
function createServer(opts) {
  return new Promise((resolve, reject) => {
    ...
    // `|| DEFAULT_PORT` here would swallow port:0 (falsy!) — epubservertest
    // passes 0 for an ephemeral port; treating it as DEFAULT made the test
    // server fight the real one on 17890 (EADDRINUSE -> 17891) with two live
    // entry/blankfix sync loops racing each other (leaked CDP ws, hanging test).
    const startPort = (opts.port === undefined || opts.port === null) ? DEFAULT_PORT : opts.port;
    ...
    let entryTimer = null;
    // Sidebar "控制中心" entry card on the ZCode main renderer page. Needs the
    // ACTUAL port (drift-aware), so it starts on successful listen, not before.
    const sidebarEntry = require("./sidebar-entry.cjs");
    let tries = 0;
    function tryListen(port) {
      ...
      server.listen(port, host, () => {
        const entryBase = "http://" + (host === "0.0.0.0" ? "127.0.0.1" : host) + ":" + server.address().port;
        sidebarEntry.sync(entryBase).catch(() => {});
        entryTimer = setInterval(() => { sidebarEntry.sync(entryBase).catch(() => {}); }, 3000);
        resolve({
          server, port: server.address().port, host, library, blankfixTimer, entryTimer,
          close: () => {
    ...
    async function handle(req, res) {
      ...
      // ---- hindsight 管理（self-hosted 模式；AGENTS.md "hindsight 管理"章节）----
      // 路径在 /api/ 前缀下 -> cdp.filterTargets 天然把本组路由排除出 CDP 注入目标。
      // 只读路由绝不 throw：服务没开时 graceful（running:false / error 字段）。
      // POST start/stop 是本子系统唯一写面：start detached 即忘（就绪靠 /health 轮询），
      // stop 端口杀优先（hindsight-embed CLI stop 在中文 Windows 崩溃的既定对策）。
      // ⚠ 测试红线：POST start/stop 不进集成测试 —— 本机 9077 跑着真实服务，stop
      // 会真的端口杀掉它。写逻辑由 hindsighttest.cjs 纯函数覆盖（parseNetstatPids、
      // startServer 拒非法 profile 名）。
      if (p.indexOf("/api/hindsight/") === 0) {
        const rt = hindsight.resolveRuntime("claude-code");
        if (p === "/api/hindsight/status" && method === "GET") {
          const health = await hindsight.checkHealth(rt.view.apiUrl);
          const embedAvailable = await hindsight.hasHindsightEmbed();
          return sendJson(res, 200, {
            serverMode: rt.view.serverMode, apiUrl: rt.view.apiUrl, apiPort: rt.view.apiPort,
            profile: rt.profile, profileByPort: rt.byPort,
            running: health.running, health: health.running ? health.health : null,
            version: health.version || null, error: health.error || undefined,
            embedAvailable, paths: rt.paths,
          });
        }
        if (p === "/api/hindsight/config" && method === "GET") {
          return sendJson(res, 200, rt.report);
        }
        if (p === "/api/hindsight/banks" && method === "GET") {
          return sendJson(res, 200, await hindsight.listBanks(rt.view.apiUrl));
        }
        const hm = /^\/api\/hindsight\/banks\/([^/]+)\/pages$/.exec(p);
        if (hm && method === "GET") {
          return sendJson(res, 200, await hindsight.bankPages(rt.view.apiUrl, decodeURIComponent(hm[1])));
        }
        if (p === "/api/hindsight/logs" && method === "GET") {
          const lines = parseInt(u.searchParams.get("lines") || "200", 10);
          const n = (Number.isFinite(lines) && lines > 0) ? Math.min(lines, 1000) : 200;
          return sendJson(res, 200, {
            pluginLog: hindsight.tailFile(rt.paths.pluginLog, n),
            daemonLog: hindsight.tailFile(rt.paths.daemonLog, n),
          });
        }
        if (p === "/api/hindsight/server/start" && method === "POST") {
          return sendJson(res, 200, hindsight.startServer(rt.view, rt.profile));
        }
        if (p === "/api/hindsight/server/stop" && method === "POST") {
          return sendJson(res, 200, await hindsight.stopServer(rt.view, rt.profile));
        }
