...
// Glued-heading guard (偷香高手 type, 2026-09): the chapter marker glued to
// the END of a body paragraph ("……你是不是喜欢我？"第九十八章以目为剑). The
// prefix before the marker is running prose, so it contains strong body
// punctuation. Legitimate prefixes (卷一 烽火连三月 / 正文 / 外传 / 江湖篇)
// never do. Only fires when the marker is NOT at line start, so 行首标题
// (incl. indented ones) are untouched.
const BODY_PUNCT_RE = /[，。！？；：、“”‘’「」『』（）()《》…—]/;
// Heading lines are SHORT. A "第X章" match on a 40+ char line is a body
// paragraph (kookit/koodo-reader uses the same length cap in isTitle).
// Real headings are far below this; 回到明朝-style "卷一 长卷名 第一章 题目"
// is ~20 chars.
const HEAD_MAX_LEN = 40;

// Special headings WITHOUT a 第X[章节回] marker ("番外 一 石头记", "尾声",
// "楔子"). kookit recognizes these as standalone heading lines via a
// word list; we do the same but stricter — the word must start the line AND
// what follows must be nothing / whitespace / a numeral / bracket (so
// "番外篇将在下周更新" or "前言不搭后语" don't match), no body punctuation,
// short line. Mid-book 楔子/番外/尾声 currently only get caught at the book's
// head (preface) or in the last chapter (afterword) — this closes the middle.
const SPECIAL_HEADS = [
  "序章", "楔子", "引子", "前言", "序言", "引言",
  "后记", "后序", "尾声", "番外", "终章", "结局",
];
// Returns the matched head word, or null. Pure function.
function specialHeadMatch(line) {
  if (!line || line.length >= HEAD_MAX_LEN) return null;
  for (const word of SPECIAL_HEADS) {
...
function findAfterword(lastBody) {
  ...
  for (const raw of lines) {
    const line = raw.trim();
    // The FIRST line is the last chapter's own heading. With special-head
    // recognition the last chapter may BE "尾声"/"番外 X" — its heading line
    // matches AFTERWORD_CANDIDATES and would re-split an empty chapter.
    if (lineStart === 0) {
    ...
    for (const { word, title } of AFTERWORD_CANDIDATES) {
      if (lineMatchesGuard(line, word)) {
        ...
        if (wordIdxInRaw >= 0) {
          // 最小余文守卫：标记词之后到文末须有实质内容（>=20 字符），否则
          // "（全文完）"只是完结标记最后一行，切出空章无意义。
          const remainder = lastBody.length - (lineStart + wordIdxInRaw + word.length);
          if (remainder < 20) return null;
...
function parseTOC(text, filename) {
  ...
  for (const raw of lines) {
    ...
    // Guarded lines (marker mid-line + body punctuation in prefix) are pure
    // body: no chapter AND no volume title (else the whole prose line would
    // land in `volumes` via the volMatch branch below). Same for a 40+ char
    // line: headings are short (HEAD_MAX_LEN, kookit-style).
    ...
    // Special heading fallback ("番外 一 石头记", "尾声", "楔子") — only on
    // lines with NO 第X章 marker at all.
    ...
    if (volMatch) {
      ...
      // Same shape of impurity: author NOTES that start with a vol marker
      // ("第三卷金蛇风云到此结束，下一章开始新的一卷" in 偷香高手). Real vol
      // titles are short noun phrases without sentence punctuation. Left
      // unfiltered it became the book's ONLY volume and (with the old
      // renderToc) ate chapters 0..548 out of the TOC.
