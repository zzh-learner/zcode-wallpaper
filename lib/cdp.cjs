...
module.exports = { filterTargets, listTargets, listAllTargets, httpGetJson, connect, fixWsHost, classifyWallpaperDom, probeWallpaperMode, probeSkinMode, PORT, HOST };
...
// Probe one page target's skin state (spec §6.2). Returns {applied, themeName}.
// applied=true if #zcode-user-skin exists; themeName from its data-theme-name attr.
// Throws on CDP/connect failure (caller catches -> null/unknown). Mirrors
// probeWallpaperMode shape. SKIN_STYLE_ID mirrors skin-selectors.cjs (kept in
// sync manually; skininjecttest mirrors to detect drift).
const PROBE_SKIN_STYLE_ID = "zcode-user-skin";
async function probeSkinMode(target) {
  const { ws, call } = await connect(target.webSocketDebuggerUrl);
  try {
    const r = await call("Runtime.evaluate", {
      expression: "(function(){var s=document.getElementById(" + JSON.stringify(PROBE_SKIN_STYLE_ID) +
        ");return JSON.stringify({applied:!!s,themeName:s?s.getAttribute('data-theme-name'):null});})()",
      returnByValue: true,
    });
