...
const inject = require("./inject.cjs"); // VIDEO_EXTS shared with the picker, not duplicated
const hindsight = require("./hindsight.cjs"); // 记忆服务探测（self-hosted 模式）
...
function mergeProbeResults(parts) {
  const probeErrors = [];
  for (const k of ["zcode", "wallpaper", "transparent", "reader", "resources", "rotate", "hindsight"]) {
...
async function probeResources(root) {
  return {
    ...
    videos: _listByExt(path.join(root, "wallpapers-video"), inject.VIDEO_EXTS).length,
    novels: (function () { try { return fs.readdirSync(path.join(root, "novels")).filter(n => /\.(txt|epub)$/i.test(n)).length; } catch (e) { return 0; } })(),
...
async function probeZcodeAndWallpaper() {
  ...
  let skinApplied = false, skinThemeName = null;
  for (const t of pages) {
    try {
      const r = await cdp.probeWallpaperMode(t);
      if (r.mode !== "none") { mode = r.mode; videoMuted = r.videoMuted; }
    } catch (e) { /* per-target fail, continue */ }
    // skin probe: once we find an applied skin on any target, record it
    if (!skinApplied) {
      try {
        const sk = await cdp.probeSkinMode(t);
        if (sk.applied) { skinApplied = true; skinThemeName = sk.themeName; }
      } catch (e) { /* per-target fail, continue */ }
    }
  }
  return {
    ...
    skin: { applied: skinApplied, themeName: skinThemeName },
  };
}
...
// ---- hindsight probe（self-hosted 记忆服务；self-hosted 模式下没人自动拉起，
// 总览给出可见状态正是本项的价值。探查失败不致命，走 probeErrors）----
async function probeHindsight() {
  const rt = hindsight.resolveRuntime("claude-code");
  const health = await hindsight.checkHealth(rt.view.apiUrl);
  return {
    running: health.running,
    serverMode: rt.view.serverMode,
    apiUrl: rt.view.apiUrl,
    profile: rt.profile,
  };
}
...
async function snapshot(opts) {
  ...
  // zcode + wallpaper + skin (CDP; null if down)
  try { const zw = await probeZcodeAndWallpaper(); parts.zcode = zw.zcode; parts.wallpaper = zw.wallpaper; parts.skin = zw.skin; }
  catch (e) { parts.zcode = null; parts.wallpaper = null; parts.skin = null; parts.zcodeError = e.message; }
  // transparent
  ...
  // hindsight (memory server; null if probe throws)
  try { parts.hindsight = await probeHindsight(); }
  catch (e) { parts.hindsight = null; parts.hindsightError = e.message; }
