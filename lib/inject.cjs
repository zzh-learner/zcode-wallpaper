...
const VIDEO_EXTS = [".mp4", ".m4v", ".webm", ".mov", ".ogg", ".ogv"];
...
function buildVideoExpression(css, videoUrl, tuneCss) {
  return (
    ...
    // Manage the glass-tune layer (clear old; rebuild only when tuneCss given).
    tuneStep(tuneCss) +
...
async function main() {
  ...
  if (MODE === "video") {
    ...
    if (process.env.ZCODE_WP_VIDEO) {
      ...
      if (videos.length === 0) {
        ...
        console.log("[wallpaper] 把 .mp4/.m4v/.webm/.mov 放进 wallpapers-video/ 后重跑。");
        console.log(
  ...
  function verifyExpression(mode) {
    if (mode === "remove") {
      // 'present' if ANY of the style / video element / glass-tune layer is
      // still around -- a clean --remove must clear all three (image mode
      // leaves a style, video mode leaves a video, a tuned session leaves the
      // tune layer).
      return (
