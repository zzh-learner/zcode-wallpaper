...
function renderSkinCss(theme) {
  ...
  if (ov.enabled) {
    ...
    if (ov.panelBg) {
      ...
      if (pr) overlayRules.push(".bg-surface, .bg-background-alt, .bg-background-win-alt { background-color: " + pr + " !important; }");
    }
    if (ov.inputBg) {
      var ir = skinModel.hexToRgba(ov.inputBg, ov.inputOpacity);
      if (ir) overlayRules.push(".bg-input, .focus-within\\:bg-input-focused { background-color: " + ir + " !important; }");
    }
    if (ov.sidebarBg) {
...
function renderSkinChromeCss(theme) {
  ...
  // hardcoded 6 fixed positions. Opacity varies per-particle for depth.
  if (d.sparkle) {
    var count = isFinite(d.sparkleCount) && d.sparkleCount != null ? Math.max(0, Math.min(50, Number(d.sparkleCount))) : 12;
    for (var i = 0; i < count; i++) {
      ...
      var opacity = 0.35 + ((i * 53) % 60) / 100; // 0.35 - 0.94
      rules.push("#" + sel.SKIN_CHROME_ID + " .skin-sparkles i:nth-child(" + (i + 1) + ") { " +
        "left: " + leftPct.toFixed(1) + "%; top: " + topPct.toFixed(1) + "%; opacity: " + opacity.toFixed(2) + "; }");
    }
  }
