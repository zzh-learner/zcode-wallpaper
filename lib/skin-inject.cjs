...
// Hex fallbacks for FROST_BASE_VARS (spec §10.4: var(name, fallback) guards
// against ZCode upgrading and renaming a var. Spike-observed theme values,
// roughly matching dark theme. If the var resolves, fallback is never used.
var FROST_FALLBACK_HEX = {
  "--color-neutral-900": "#121216",
  "--color-input": "#2b2b2b",
...
function renderSkinCss(theme) {
  ...
  if (ov.enabled) {
    ...
    for (var i = 0; i < regions.length; i++) {
      ...
      var colorMix = "color-mix(in srgb, var(" + r.baseVar + ", " + (FROST_FALLBACK_HEX[r.baseVar] || "#121216") + ") " + opPct + "%, transparent)";
