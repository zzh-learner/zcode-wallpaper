...
// registers (and leaves dangling) ws connections after teardown. Without this,
// a sync that races close() drops a live CDP ws into `registered` AFTER close
// emptied it — nobody ever closes it and the node event loop stays alive,
// hanging tests that rely on natural exit (exposed 2026-09-14 when tests ran
// with real webview targets on :9222). Mirrors sidebar-entry.cjs closedFlag.
let closedFlag = false;
// re-entrancy guard (mirrors sidebar-entry.cjs): a slow sync overlapping the
// next 3s tick would double-register the same target — two ws for one Map
// key, the first becomes an orphan nobody ever closes.
let syncing = false;
