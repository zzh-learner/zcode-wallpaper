...
function scopeChunk(text, prefix) {
  ...
  while (i < text.length) {
    ...
    // epub `body{...}` / `html{...}` set the chapter's global font/color/margin.
    // But the chapter fragment has no <body>/<html> (sanitizeChapterXhtml strips
    // them; only the body's children are inserted under #epub-content). A naive
    // `#epub-content body{...}` would never match. Map a bare body/html selector
    // to the container itself; anything else (p, .x, body p, body.foo, .body)
    // is prefixed normally. Case-insensitive whole-word match only.
    const scoped = selectorList.split(",").map(s => {
      const t = s.trim();
