...
**真机验证结果（Task 16 全部通过，2026-06-19）：**
- ✅ webview 能加载 `http://localhost:17890/reader/`（具体端口）— CDP 确认 globals 全 object、书架显示书、fetch 200
- ✅ webview 里拖拽 `.txt` 能拿到 `File` 对象（拖拽模式命门）— 用户实测可用
- ✅ webview `localStorage` 在 persist partition 下持久（进度持久化命门）— localStorage 存了 chapterIndex=2003，刷新自动回到该章
- ✅ server 启动后剪贴板 URL 能粘进 ZCode 浏览器面板 — 启动后剪贴板内容正好是 server URL
- ✅ 章节导航（目录点击、←/→ 翻章）— CDP 模拟点击验证：目录 12 卷 2004 章正常、翻章切换正常

**附带发现的非阻塞遗留（不在原 spec 风险列表里，真机新发现）：**
- 最后一章"第一千九百九十九章 黑日"吞了 147 万字后记/番外（无"第X章"标题的内容被并进最后一章，29278 段）。只影响最后一章显示，不阻塞主流程。修复思路：parseTOC 末章 endOffset 用"下一个明显分界"（如"（全书完）"、"——后记——"）而非 text.length，留作 v2。
- 调试发现：CDP `/json` 返回的 webview wsDebuggerUrl 是 `ws://localhost/devtools/...`（无端口，默认 80 连不上）。任何脚本连 webview 都要重写成 `ws://127.0.0.1:9222/...`。已记进 `scripts/inspect-reader.cjs`。
