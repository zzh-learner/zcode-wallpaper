...
四种能力：**图片壁纸**、**视频壁纸**（把 `.mp4` 当动态背景播放）、**窗口透明**（让 ZCode 主窗口半透明，能透过它看到桌面）、**小说阅读器**（在 ZCode 浏览器面板里看本地 .txt 小说，带目录/书架/进度）。
前三者是改 ZCode 外观，阅读器是独立子应用 —— 详见各章节。

> 🆕 **控制中心**（`start.bat`）：带界面的统一控制台，在一个透明 webview 面板里
> 实时显示状态 + 一键操作壁纸/视频/透明/阅读器。**日常推荐双击 `start.bat`**，
> 而不是逐个跑菜单场景。详见下方「控制中心」章节。
...
- **控制中心**（`start.bat`）：带界面的统一控制台——透明 webview 面板（能透出壁纸），实时显示 ZCode/壁纸/透明/阅读器/资源状态，一键操作所有功能，带书架管理（跳转/删除/加书）。
...
> 💡 **日常推荐**：双击 **`start.bat`**（一站式：调试模式启动 ZCode + 起控制中心），
...
双击项目根目录的 **`start.bat`**（一站式入口）。它会：

1. 预检 Node.js
2. **以调试模式重启 ZCode**（带 `--remote-debugging-port=9222`，会先关掉当前 ZCode——这是 CDP 的必须代价）
3. 启动控制中心 server（常驻窗口，URL 自动写剪贴板）

然后在 ZCode **浏览器面板**地址栏粘贴 `http://127.0.0.1:17890/control/` 回车。
（也可以双击 `wallpaper.bat` 选 `13 启动控制中心`，只起 server 不重启 ZCode。）

> ⚠️ ZCode 必须以调试模式启动（`start.bat` 或 `wallpaper.bat` 场景 2/7/9），
...
| 文件 | 作用 |
|------|------|
| `start.bat` | **🆕 推荐入口**：双击它 = 调试模式启动 ZCode + 起控制中心（一站式） |
...
| `lib/control-server.cjs` | 🆕 合并控制中心 server（静态托管 control/+reader/ + 小说/状态/动作 API；reader-server.cjs 现委托它） |
...
| `control/` | 🆕 控制中心前端 SPA（透明背景 + 浮动控件 + 书架管理） |
...
| 控制中心打不开（webview 空白） | 确认控制中心 server 窗口还开着；端口对（17890，冲突会 +1，看窗口打印）；URL 是 `/control/`（带尾斜杠） |
