...
- **想恢复声音**：点"🔊 取消静音"。
- **切换到图片壁纸会自动停声音**：点"注入图片壁纸"时，旧视频元素连同声音一起被移除——
  不会出现"画面已经是图片、视频声音还在响"。
...
> ⚠️ 文件名建议用**纯英文、别用中文/空格**。**真机验过的容器：`.mp4` 最稳，`.mov` 实测完全可播**（iPhone 拍的 mov 直接能用）。
> 其它容器（`.webm`/`.mkv`/`.avi`/`.ogg` 等）没验过，用前请用 `scripts/inspect.cjs` + 人眼确认能播。
...
iPhone 导出的实况照片是 `.livp` 文件——它**不是单一图片，是个 zip 容器**，里面同时装着
一张 JPEG 和一段 MOV（实况视频）。本项目两个壁纸子系统都不认 `.livp` 扩展名，直接拷进来
不会被选到。用 `scripts/unpack-livp.cjs` 一键拆开：

```bash
# 先 dry-run 看会写哪些文件（默认，不写盘）
node scripts/unpack-livp.cjs "G:\手机照片备份目录"

# 确认无误后实际写入
node scripts/unpack-livp.cjs "G:\手机照片备份目录" --apply

# 写完图片记得重新缩图（视频不用缩）
node lib/resize.cjs
```

每个 `.livp` 解出 1 张 jpg（进 `wallpapers/`）+ 1 段 mov（进 `wallpapers-video/`）。
输出文件名带 livp 源文件名前缀（如 `2026-03-02 190658__IMG_2571.jpg`），防止多个 livp 内部
都用 `IMG_XXXX` 命名导致重名覆盖。可重复跑（同尺寸文件不覆盖，不会污染缩图缓存）。

> ℹ️ 脚本零外部依赖（用 Node 内置 zlib 手写 zip 解析），不碰 CDP/inject，纯文件 IO。
...
1. 把 `.txt` 或 `.epub` 小说放进 **`novels/`**（`.gitignore` 已忽略，私人内容不提交）
...
5. 从书架选书，或直接把 `.txt` 拖进阅读区（**epub 不支持拖拽**，只能放进 `novels/` 由服务加载）

> epub 仅在 server/http 模式可用（解 zip 需要文件系统 + jszip）；txt 还有拖拽兜底模式。
...
- **epub 格式**：章节 HTML/CSS/图片渲染、XSS 防护、CSS 作用域隔离不污染 reader UI。
  主题对 epub 同样生效——切主题时阅读区背景+正文色跟随，epub 作者在子元素（标题/链接）上设的
  特定颜色保留。
...
| `lib/epub.cjs` | epub 纯函数（scopeCss 作用域隔离 + body/html 映射、XSS sanitize + src 改写、目录构建、路径穿越白名单） |
| `lib/epub-load.cjs` | epub 加载胶水（jszip 解 zip、spine/nav 组装目录、章节/asset 读取） |
| `lib/wallpaper.css` | 图片壁纸样式 |
| `lib/wallpaper-video.css` | 视频壁纸样式（视频层定位 + 透明 UI 层） |
| `reader/` | 阅读器前端 SPA（HTML/CSS/JS，支持 txt + epub 双格式；txt 双模式：server fetch / 拖拽兜底，epub 仅 server 模式） |
...
| `scripts/unpack-livp.cjs` | 🆕 解包 Apple Live Photo `.livp` 容器（zip，内含 1 JPEG + 1 MOV）→ jpg 进 `wallpapers/`、mov 进 `wallpapers-video/`。默认 dry-run，`--apply` 才写盘；按 livp 时间戳前缀重命名防 `IMG_XXXX` 重名 |
