...
| 移除注入 | `npm run remove` |
| 装/重装 sharp/ws 依赖 | `npm run setup` |
| 起小说阅读器 server | `npm run reader` |
| 起控制中心 server | `npm run control` |

**`pretest` 钩子（重要，容易漏）**：`npm test` 实际跑 `npm run pretest`（`node test/fixtures/make-epub.cjs`）
重新生成 `test/fixtures/normal.epub`（epub 测试的真实 zip fixture）再跑测试。**直接单跑某个
`node test/epub*.cjs`（绕过 `npm test`）时，若 fixture 不存在或过时，epubloadtest/epubservertest 会失败**——
先 `npm run pretest` 或直接跑 `node test/fixtures/make-epub.cjs` 重建它。fixture 在 `.gitignore` 里（运行时产物）。
...
- **章节识别覆盖 ~95% 常见格式，剩 4 类罕见格式不支持**（批量测 86 本起点完结小说，
  ...
  5. **段尾粘连假章节（2026-09-16 已修，粘连守卫）**：偷香高手型——章节标题
     **粘在正文段落末尾同一行**（`……你是不是喜欢我？"第九十八章以目为剑`），
     第98章被切成 6 个重复目录条目。守卫：标记不在行首（chapIdx>0）且**前缀**
     含正文标点（`BODY_PUNCT_RE`）→ 判为粘连段落，不算章节也不提卷标题。
     行首标题（含缩进）与合法前缀（卷一/正文/外传，无标点）不受影响。
     **意外收益**：顺手消掉十国千娇（26 例）/枭臣（11 例）的口语"第一回见面"
     假阳性（前缀都带标点）——即单位投票方案（见 memory
     reader-toc-unit-voting-plan）的主体，该方案因此暂缓。
     **已知代价**（测试钉死）：卷名带标点+卷章同行的真标题会被误杀（判比粘连
     段落罕见）；"加更第6章"型（前缀无标点）和行首正文假阳性（"第三章会很晚
     ，可以明天再看"）仍接受。server `lib/reader-toc.cjs` + mirror
     `reader/lib/toc.js` 两边同步（教训 17），用例镜像在两个 toc 测试里。
  6. **特殊标题独立行 + 标题行长上限（2026-09-16，借鉴 koodo-reader 的
     kookit 引擎 `src/libs/textProcessor.ts` isTitle）**：①行首词表
     （序章/楔子/引子/前言/后记/尾声/番外/终章/结局…）且词后是空白/数字/
     括号的**短行**（<40 字）识别为章节——补上"番外 一 石头记"这类无
     "第X章"格式的中部盲区（此前前言只管书首、后记只管末章）。比 kookit
     更严：kookit 是 startsWith 即认，我们要求词后不能直接接正文文字
     （"番外篇将更新"/"前言不搭后语"不匹配）。②标题行 <40 字上限
     （kookit 同款）：40+ 字的"第X章"行必是正文。③连带修了两个 afterword
     交互：findAfterword 跳过末章第一行（主循环识别"尾声"章后，其标题行
     会被 AFTERWORD_CANDIDATES 二次命中切出空章）；完结标记（"（全文完）"）
     之后余文 <20 字符不再切（完结标记是最后一行时切空章无意义）。
     **没抄的**：kookit 的"第"位置 <7 短前缀规则会杀"卷一 烽火连三月
     第一章"（前缀 8 字），我们用标点守卫替代；kookit 不分卷/章两级全
     平铺，我们保留两级；500 行兜底分章、用户自定义正则——YAGNI。
...
### body/html 选择器映射到容器（不是简单加前缀）

epub CSS 几乎都用 `body{...}` 设全局字体/颜色/行距（标准开头）。但章节 DOM 里**没有 `<body>`/
`<html>` 元素**——`sanitizeChapterXhtml` 剥掉了 epub 的 `<body>` 标签，`reader.js` 只把 body 的
子节点片段插入 `#epub-content`。所以如果 scopeCss 简单加前缀成 `#epub-content body{...}`，这条规则
**永远匹配不到任何元素**（容器内无 body 子元素），epub 作者的全局样式全丢——读者看到的是浏览器
默认黑字，作者配色/字体/行距都没生效。`html{...}` 同理。

**修复**：scopeCss 对**整词** body/html 选择器（`trim().toLowerCase()` 等于 `body`/`html`，
大小写不敏感）映射到容器本身（`#epub-content`），不加后代空格。其余选择器行为不变：
- `body` → `#epub-content` ✓（匹配容器本身）
- `html` → `#epub-content` ✓
- `p` / `.x` / `ul li` / `#cover` → `#epub-content p` 等（照常加前缀）
- `body p`（后代组合）→ `#epub-content body p`（**不**映射——p 是容器真实后代，照常加前缀）
- `body.night`（复合）→ `#epub-content body.night`（不映射，罕见，YAGNI）

边界只处理 trim 后**整词等于** body/html 的。`body.foo`/`body p`/`.body` 不动——它们要么本就
能匹配（后代 p 是真实元素），要么是作者针对真 body 的特例（罕见，YAGNI 不特殊处理）。

### epub 主题色优先（背景+默认文字跟随主题，覆盖作者色）

epub 容器 `#epub-content`（`reader/reader.css`）设 `background: var(--c-bg) !important;
color: var(--c-fg) !important;`。切主题（`.theme-dark`/`.theme-light`/`.theme-sepia`）时，
epub 阅读区的**背景 + 默认文字色**跟随主题，**覆盖** epub 作者在 `body{color/background}` 上设的颜色。

**为什么用 `!important`**：scopeCss 注入的 `<style>`（含 `#epub-content{color:#333}` 来自作者
body 声明）在 reader.css 之后加载，选择器特异性同为 `#epub-content`（1 个 id），按 CSS 规则后者
（作者色）会赢。`!important` 让 reader.css 的主题色胜出。这是有意的覆盖（用户选「主题色优先」）。

**只管容器层，子元素作者色保留**：`!important` 只设在 `#epub-content` 容器本身。epub 作者在
子元素上设的颜色（`h1{color:blue}`、`a{color:green}`、`.chapter-text{color:#333}`）照常生效——
scopeCss 后是 `#epub-content h1{color:blue}`，子元素的 color 声明优先于从容器的继承，作者表达
保留。作者的**非颜色样式**（`font-family`/`line-height`/`text-indent`）不受 `!important` 影响，
正常生效。
...
- **主题色只覆盖容器层**：epub 作者在子元素（标题/链接/强调 class）上设的特定颜色保留作者原色，
  不强制跟随主题（用户选「只管容器层」）。如某本 epub 在每个 `<p>` 上设了颜色，正文不会跟主题——
  那是作者意图。要全强制可加 `#epub-content *{color:inherit}` 但会抹掉所有作者配色，YAGNI 不做。
- **真机验证清单（spec §6.3）需 webview 实跑**：书架显示 epub、章节渲染+图片+CSS、恶意 epub 不弹窗、
  epub CSS 不污染顶栏、进度保存恢复、看完 epub 切 txt 正常（epub-mode class 移除）、**切主题时
  epub 阅读区背景+正文色跟随变化而标题/链接作者色保留**。这些是 DOM 渲染层，单测盲区（教训 12），靠真机验。
...
### v2 修复：fresh-mount 竞态与一次性提交的失败（2026-09-14 晚，用户真机抓到）

v1 点击链是"填值 + `requestSubmit()` **一次**"。用户真机路径"关掉整个侧边面板 →
重开 → 点卡"翻车：面板开了、地址栏填了、webview 也创建了（src 正确），**但 webview
永远 0 宽**——提交落在浏览器面板 fresh mount 的初始化窗口里，面板 init 随后重置了
tab 状态，留下孤儿 webview。真机探明的关键事实：

- **僵尸 webview 连"关面板→重开"都清不掉**（pane toggle 不 unmount 它，关标签也不清），
  页面 DOM 被污染到 ZCode 重启为止——点卡后卡在这种半死状态，地址栏 hidden、卡片列表
  也不再渲染。
- v1 成功的两次 E2E 都是从"面板已挂载的空状态"出发的（先关过标签）；用户路径是
  "面板从未挂载 → 点卡触发 mount"，两场景的竞态窗口不同。**测试路径没覆盖用户真实
  路径**（教训 7 同型：边界要测用户真实操作序列）。

**v2 修复（`ENTRY_VERSION=2`）**：点击链改**重试+验证**——每 250ms 检查"是否存在
src=目标 URL 且宽度>0 的 webview"，没有就重新填值+提交（幂等），成功即停，12 秒
放弃。首次提交被 mount 竞态吞掉后，下一次重试落在已初始化的面板上就成功。地址栏
不可见时跳过提交（等面板挂载完）。**版本接管**：卡片带 `data-zz-entry-v` 戳，
install 发现旧版本卡就移除重建——旧 source 的 addScriptToEvaluateOnNewDocument 注册
在页面上活到 ZCode 重启，靠版本戳让新代码在活页面接管（教训 1 同型：两份注入源
并存时必须有单一权威判定谁拥有卡片）。

**v3 修复（同日晚，用户真机抓到第二层）**：v2 修好"本会话"后，用户发现**切到其他会话
点卡仍失败**（"打开了浏览器然后没有下文"）。真机探测发现根因是**ZCode 侧边面板按会话
隔离、多实例并存**：切换会话后其他会话的浏览器面板（含自己的地址栏 input）仍留在 DOM
里——实测同屏 2 个 `browser-address-input`（活跃会话的可见 + 隐藏会话的空值）。v1/v2 用
`document.querySelector` 拿**DOM 序第一个** input：在别的会话里第一个属于隐藏面板——
v1 把地址填进了看不见的输入框（提交了个寂寞），v2 的"等可见"永远等不到（它等的对象
错了）。v3 三处修复：**遍历全部 input 取可见的那个**；dispatch 浏览器卡限定在
`card.closest('.side-pane-open-tab-list')` 自己的列表（不打扰别的会话的卡）；install
优先可见列表。版本接管机制让 v3 在活页面直接替换 v2 的卡，**无需重启 ZCode**。

**v3 真机验证（2026-09-14 深夜，`scripts/verify-entry-multisession.cjs`）**：自动驱动
"切到其他会话 → 等 v3 卡接管 + URL 就位 → 点卡"，时间线证实 click+0.8s 该会话的可见
输入框已填 control URL、新 webview 导航到 `/control/` 且 717px 可见——v1/v2 均失败的
同路径修复生效。

**附带发现（ZCode 自身行为，非本功能所致）**：切换会话时 ZCode 的浏览器面板可能整体
降级——tab 创建后永 0 宽、地址栏消失、pane toggle 和关标签都救不回，只有重启 ZCode
恢复。自动验证脚本切过去再切回后本会话面板就坏了（v3 修复前的僵尸 tab 同款症状，
但这次由会话切换本身触发，与入口卡片无关）。**跨会话驱动 ZCode UI 的脚本要预料到
这个代价；用户侧遇到"面板空白/点不出的 tab"先想到重启 ZCode（start.vbs）。**

教训补丁：
32. **驱动外部 UI 的"一次性动作"必须带结果验证 + 重试**。submit/dispatch/click 这类
    触发式操作对"目标正处于初始化/过渡态"没有任何保障——成功触发 ≠ 生效。验证要用
    **最终用户可见的效果**（webview 有宽度、元素真的出现），不能用中间信号（"提交方法
    调了""input 值设上了"——僵尸状态里这两样全都是对的，唯独没有下文）。和教训 3
    （不信 exit code 信真实 DOM 状态）同型，这次是它的提交版。
33. **测试路径必须覆盖用户真实操作序列**。v1 两次 E2E 全绿是因为都从"面板已挂载"出发；
    用户的"关面板→重开→点卡"（触发 fresh mount）从没测过。同型于教训 7（回归测试要测
    到用户真实边界），但这次的边界是**操作历史**不是组件边界。
34. **ZCode 主页面是"多实例面板"拓扑，全局 querySelector 单例假设不成立。** 每个会话
    有自己的侧边面板实例（地址栏/空状态列表/webview 都可能同时存在多份，隐藏的留在
    DOM）。任何"找元素→操作"都要按**可见性过滤**或按 **closest 限定作用域**，别拿
    DOM 序第一个。教训 21（"应该能 X"是假设）的 DOM 拓扑版：这次"页面上只有一个地址栏"
    就是个没人验证过的假设，v1/v2 两版都栽在它上面。
...
**主题色覆盖**（`reader/reader.css` 的 `#epub-content{background/color:!important}`）是 CSS 层，单测
盲区（教训 12），靠真机验切主题效果。
...
- **仓库根有个未追踪的 `nul` 文件**（Git Bash 里 `>nul` / `2>nul` 重定向误建，已在 `.gitignore`
  里忽略，正常 `ls` 会看到）。它就是上面那条"Windows 保留名"坑的产物——**别尝试用 `Read` 读它或
  `cat nul`**（会读空/报设备错），也别误删，留着无害（gitignore 已覆盖）。需要真删时按上面那条用
  `\\?\` 前缀 + `.ps1 -File`。
